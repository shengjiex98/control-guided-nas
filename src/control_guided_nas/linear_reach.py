...
def reach(
    ...
    *,
    ...
    for k in range(1, horizon + 1):
        ...
        if return_pipe:
            pipe.append(current)
        max_diam = max(max_diam, diameter(current, nx))
...
def get_max_diam(
    ...
    *,
    ...
    x0 = Zonotope(
        np.concatenate([x0center, np.zeros(nu)]),
        np.diag(np.concatenate([x0size, np.zeros(nu)])),
    )

    def w_fn(k, x):
        if k % latency_ms == 0:
            return _error_bound(k_gain, errors, x, nx, nu)
        return Zonotope(np.zeros(nx + nu), np.zeros((nx + nu, nx + nu)))

    return reach(phi, x0, w_fn, 1000, nx, return_pipe=return_pipe)
